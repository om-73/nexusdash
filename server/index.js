...
if (process.env.SPAWN_PYTHON === 'true') {
    const { spawn } = require('child_process');

    // Logs buffer
    recentLogs = "Initializing...";
    ...
    console.log('[Orchestrator] Spawning Python Engine...');

    pythonProcess.on('error', (err) => {
        const msg = `\n[FATAL] Failed to spawn Python process: ${err.message}. (Is 'python' command available?)`;
        console.error(msg);
        recentLogs += msg;
    });
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        console.log(`[Health] Checking Python engine at: ${pythonUrl}`);
        const pythonRes = await axios.get(`${pythonUrl}/`, { timeout: 10000 });
        res.json({
            status: 'healthy',
            server: 'online',
...
app.get('/api/debug/logs', (req, res) => {
    res.json({
        ...
        env: {
            ...
            SPAWN_PYTHON: process.env.SPAWN_PYTHON,
            PYTHON_URL: process.env.PYTHON_ENGINE_URL || 'http://127.0.0.1:8000'
        }
    });
});
