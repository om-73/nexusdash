...
import { Loader, Target, BarChart2, Zap, TrendingUp, Activity, RefreshCw } from 'lucide-react';
    ...
    return (
        <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 md:space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-200 pb-6">
                <div>
                    <h1 className="text-2xl md:text-3xl font-extrabold text-slate-800 tracking-tight">Exploratory Analysis</h1>
                    <p className="text-slate-500 mt-1">Visualize distributions and relationships in your data</p>
                </div>
                <div className="flex gap-2 bg-slate-100 p-1.5 rounded-xl border border-slate-200">
                    <button
                        onClick={() => setActiveTab('univariate')}
                        className={`px-4 md:px-6 py-2 md:py-2.5 rounded-lg font-medium text-sm transition-all duration-200 flex items-center gap-2 ${activeTab === 'univariate' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'}`}
                    >
                        <BarChart2 size={18} /> <span className="hidden sm:inline">Univariate</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('bivariate')}
                        className={`px-4 md:px-6 py-2 md:py-2.5 rounded-lg font-medium text-sm transition-all duration-200 flex items-center gap-2 ${activeTab === 'bivariate' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'}`}
                    >
                        <TrendingUp size={18} /> <span className="hidden sm:inline">Bivariate</span>
                    </button>
                    <button
                        onClick={() => setActiveTab('correlation')}
                        className={`px-4 md:px-6 py-2 md:py-2.5 rounded-lg font-medium text-sm transition-all duration-200 flex items-center gap-2 ${activeTab === 'correlation' ? 'bg-white text-primary shadow-sm ring-1 ring-slate-200/50' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'}`}
                    >
                        <Activity size={18} /> <span className="hidden sm:inline">Correlation</span>
                    </button>
                </div>
            </div>

            {/* Content Area */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col md:flex-row min-h-[600px] relative">
                {/* Overlay Loader */}
                {loading && (
                    <div className="absolute inset-0 z-50 bg-white/60 backdrop-blur-sm flex items-center justify-center">
                        <div className="bg-white p-4 rounded-2xl shadow-xl flex items-center gap-3 border border-slate-100">
                            ...
                            <span className="font-semibold text-slate-700">Generating Plot...</span>
                        </div>
                    </div>
                )}
                <div className="w-full">
                    {/* Common AI Insights Section (always at top) */}
