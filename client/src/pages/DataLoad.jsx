...
import { uploadFile, loadData, connectDatabase, deleteFile } from '../services/api';
...
export default function DataLoad() {
    ...
    const handleFile = async (file) => {
        ...
        try {
            ...
            // Detect file type
            const ext = uploadRes.originalName.split('.').pop().toLowerCase();
            const fileType = (ext === 'xlsx' || ext === 'xls') ? 'excel' : (ext === 'zip' ? 'zip' : 'csv');

            const dataRes = await loadData(uploadRes.filePath, fileType);
            console.log('[DataLoad] Data loaded successfully:', dataRes);

            // Removed Auto-Cleanup: Keep the file so it persists across the session
    ...
    return (
        <div className="p-4 md:p-8 max-w-4xl mx-auto border-t border-slate-100">
            <div className="mb-6 flex justify-between items-center flex-wrap gap-4 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                <div>
                    <h1 className="text-3xl font-extrabold text-slate-800 mb-1">Upload Dataset</h1>
                    <p className="text-slate-500">Supported formats: CSV, Excel (.xlsx, .xls), ZIP</p>
                </div>
                <div className="text-slate-400 p-3 bg-slate-50 rounded-xl hidden sm:block">
                    <Database size={32} />
                </div>
            </div>
            ...
            <div className="bg-white p-4 md:p-8 rounded-2xl shadow-sm border border-slate-200">
                <div
                    className={`border-2 border-dashed rounded-xl p-8 md:p-12 text-center transition-all duration-200 ${dragActive ? 'border-primary bg-blue-50/50 scale-[1.01]' : 'border-slate-300 hover:border-primary hover:bg-slate-50'
                        }`}
                    ...
                    <div className="flex flex-col items-center justify-center gap-4">
                        ...
                        {loading ? (
                            ...
                            <>
                                ...
                                />
                                <p className="text-xs text-slate-400 mt-4">Supports CSV, Excel, ZIP (max 50MB)</p>
                            </>
                        )}
                    </div>
                </div>
